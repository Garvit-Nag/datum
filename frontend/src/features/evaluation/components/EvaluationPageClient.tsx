...
} from "@/features/evaluation/hooks/useEvaluation";
...
function getEvalErrorMessage(error: unknown): string {
  if (!(error instanceof Error))
    return "Evaluation failed. Check your inputs and try again.";
...
type GroundTruthParseResultType = {
  pairs: ParsedPairType[];
  error: string | null;
};
        ...
        <div className="flex items-center justify-between border-b border-border/40 px-5 py-3.5">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              Required structure for Q&amp;A pairs
            </p>
          </div>
          <button
            onClick={onClose}
...
export function EvaluationPageClient() {
  ...
  const docReady =
    docMode === "existing" ? !!selectedDocId : docStage === "ready";
      ...
      <div className="space-y-6 animate-fade-up">
        ...
        <div>
          ...
          <p className="mt-1.5 max-w-xl text-sm text-muted-foreground">
            Run the judge pipeline on your ground truth Q&amp;A file. Generates
            answers, judges them, and shows live progress.
          </p>
        </div>

        <div
          className={`grid gap-6 ${validated ? "lg:grid-cols-2 items-stretch" : "grid-cols-1"}`}
        >
          {/* ── Left Column ── */}
                ...
                <div className="flex items-center gap-3 rounded-xl border border-emerald-500/25 bg-emerald-500/5 px-4 py-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground">
                      {validated.count} questions ready
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Ground truth validated. Proceed to step 2.
                    </p>
                  </div>
                </div>
              ) : (
                            ...
                            <p className="text-xs text-muted-foreground">
                              {
                                uploadedText
                                  .split("\n")
                                  .filter((l) =>
                                    l.trim().toUpperCase().startsWith("Q:"),
                                  ).length
                              }{" "}
                              Q: lines found
                            </p>
              ...
              <div className="flex-1 rounded-2xl border border-border/50 bg-card/60 p-6 backdrop-blur-sm flex flex-col justify-between animate-fade-up">
                ...
                <div className="mt-5 flex flex-col gap-4">
                  <Button
                    onClick={handleRun}
                    disabled={!canRun}
                    className="gap-2 w-full h-10 text-[13px] font-medium"
                    variant="default"
                  >
                    ...
                    <div className="flex flex-col gap-2 text-[12px] text-muted-foreground rounded-xl border border-border/40 bg-background/50 p-3.5 shadow-sm">
                      {!progress && (
                        <p className="animate-pulse font-medium text-foreground">
                          Starting evaluation for {validated.count} question
                          {validated.count === 1 ? "" : "s"}…
                        </p>
                      )}
                      ...
                      {progress?.phase === "answers" && (
                        <>
                          <p>
                            <span className="font-medium text-foreground">
                              Generating answer {progress.completed} of{" "}
                              {progress.total}
                            </span>
                          ...
                          {progress.lastQuestion && (
                            <p className="truncate mt-1.5 text-[11px] opacity-70">
                              {progress.lastQuestion.slice(0, 80)}
                              {progress.lastQuestion.length > 80 ? "…" : ""}
                            </p>
                          )}
                        </>
                      )}
                      ...
                      {progress?.phase === "judging" && (
                        <>
                          <p>
                            ...
                            <span className="ml-1.5 animate-pulse text-primary font-medium">
                              · Running Judge…
                            </span>
                          </p>
                          <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted/60 mt-1">
                ...
                readyDocs.length === 0 ? (
                  <div className="flex h-[220px] flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-border/60 bg-background/30 transition-colors hover:border-border/80 hover:bg-muted/20">
                    ...
                    <p className="px-6 text-center text-sm text-muted-foreground">
                      ...
                      <br />
                      <button
                        className="mt-1.5 font-medium text-primary hover:underline underline-offset-4"
                  ...
                  <div className="w-full flex flex-col gap-1 rounded-xl border border-border/50 bg-background/50 p-1.5 h-[220px] overflow-y-auto custom-scrollbar shadow-inner">
                    ...
                    {readyDocs.map((d) => (
                      ...
                      >
                        <FileText
                          className={`h-5 w-5 shrink-0 ${selectedDocId === d.id ? "text-primary" : "text-muted-foreground/60"}`}
                        />
                  ...
                  {(docStage === "idle" || docStage === "error") && (
                    <div className="flex flex-col items-center w-full max-w-[280px] gap-3">
                      ...
                      <Button
                        ...
                        disabled={!pdfFile}
                        size="default"
                        className="w-full gap-2 rounded-xl h-9 shadow-sm"
                      >
            ...
            <div className="flex items-center justify-between">
              ...
              {runs.length > 1 && (
                <div className="flex items-center gap-1.5">
                  <span className="mr-1 text-[11px] uppercase tracking-[0.12em] text-muted-foreground/70">
                    Run
                  </span>
                  {runs.map((_, i) => (
                    <button
